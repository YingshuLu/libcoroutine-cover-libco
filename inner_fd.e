...
__thread char *_error_buffer = 
# 54 "defines.h" 3 4
                              ((void *)0)
# 54 "defines.h"
                                  ;
void dperror(int ret) {
    if(!_error_buffer) _error_buffer = (char *)malloc(1024);
    strerror_r(
# 57 "defines.h" 3 4
              (*__errno_location ())
# 57 "defines.h"
...
inline void list_add_before(list_t *ls, list_t *elm) {
    do { elm->prev = ls->prev; elm->next = ls; ls->prev->next = ls->prev = elm;} while(0);
}

inline void list_add_after(list_t *ls, list_t *elm) {
    do { elm->prev = ls; elm->next = ls->next; ls->next = ls->next->prev = elm;} while(0);
}
...
inline void list_delete(list_t *elm) {
   do { elm->prev->next = elm->next; elm->next->prev = elm->prev;} while(0);
}
...
inner_fd* g_inner_fd_list[102400] = {0};

bool is_fd_valid(int fd) {
    return (fd > -1 && fd < 102400);
}
